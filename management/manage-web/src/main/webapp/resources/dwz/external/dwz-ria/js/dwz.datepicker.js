		...
		return this.each(function(){
			...
			function generateCalendar(dp){
				...
				if (dp.hasTime()) {
					...
					var dwHour = dw.hour;
					if($this.hasClass('dateInitStart')){
						dwHour = dwTime ? dwTime.substring(11,13) : 0;
					}else if($this.hasClass('dateInitEnd')){
						dwHour = dwTime ? dwTime.substring(11,13) : 23;
					}
					var $hour = $(setting.hour$).val(dwHour).focus(function(){
						changeTmMenu("hh");
					});
					var iMinute = parseInt(dw.minute / dp.opts.mmStep) * dp.opts.mmStep;
					var dwIMinute = iMinute;
					if($this.hasClass('dateInitStart')){
						dwIMinute = dwTime ? dwTime.substring(14,16) : 0;
					}else if($this.hasClass('dateInitEnd')){
						dwIMinute = dwTime ? dwTime.substring(14,16) : 59;
					}
					var $minute = $(setting.minute$).val(dwIMinute).attr('step',dp.opts.mmStep).focus(function(){
						changeTmMenu("mm");
					});
					var dwSecond = dp.hasSecond() ? dw.second : 0;
					if($this.hasClass('dateInitStart')){
						dwSecond = dwTime ? dwTime.substring(17,19) : 0;
					}else if($this.hasClass('dateInitEnd')){
						dwSecond = dwTime ? dwTime.substring(17,19) : 59;
					}
					var $second = $(setting.second$).val(dwSecond).attr('step',dp.opts.ssStep).focus(function(){
						changeTmMenu("ss");
					});
