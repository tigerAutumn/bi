...
public class TransDetailFacade {
	...
	public void queryByUserId(ReqMsg_TransDetail_QueryByUserId req,ResMsg_TransDetail_QueryByUserId resp){
		...
		if(list != null && list.size() > 0){
			...
			for (BsUserTransDetail detail : list) {
				...
				if(detail.getTransType().equals(Constants.Trans_TYPE_TOP_UP)){
					...
					mapDetail.put("transName", "回款");
					mapDetail.put("transTime", detail.getCreateTime());
		...
		resp.setTotalCount((int)Math.ceil((double)transDetailService.findByUserIdCount(req.getUserId())/pageSize));
	...
	@InterfaceVersion("QueryByUserId/1.0.1")
	public void queryByUserId1_0_1(ReqMsg_TransDetail_QueryByUserId req,ResMsg_TransDetail_QueryByUserId resp){
	...
	@InterfaceVersion("QueryZanReturnDetail/1.0.0")
