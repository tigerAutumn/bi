package com.pinting.gateway.in.hessian;
 ...
 */
public class GatewayMobileHessianServiceImpl extends HessianBaseService implements HessianService , ApplicationContextAware{
	
	protected static ApplicationContext applicationContext;
	private static ValidatorFactory validatorFactory = Validation.buildDefaultValidatorFactory(); 
	
	private Logger log = LoggerFactory.getLogger(GatewayMobileHessianServiceImpl.class);
	...
	public ResMsg handleMsg(ReqMsg reqMsg) {
		...
		try {
			...
			Object bsFacade = applicationContext.getBean("app"+facadeName);
			if (bsFacade == null) {
			...
			for (Method method : bsFacade.getClass().getDeclaredMethods()) {
				//判断方法上是否有指定类型的注解
				Object obj = AnnotationUtils.findAnnotation(method, InterfaceVersion.class);
				if(obj instanceof InterfaceVersion) {
					InterfaceVersion annotation = (InterfaceVersion) obj;
					String value = annotation.value();
					String type = value.split("/")[0];
					String version = value.split("/")[1];
					if(actionName.equals(type) && (version.equals(reqMsg.getVersion()) || "inner_0.0.0_".equals(version))) {
						try {
	...
	public void setApplicationContext(ApplicationContext applicationContext)throws BeansException {
		GatewayMobileHessianServiceImpl.applicationContext = applicationContext;
