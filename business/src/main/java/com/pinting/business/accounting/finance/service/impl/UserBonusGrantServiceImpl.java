 ...
 */
@Service
public class UserBonusGrantServiceImpl implements UserBonusGrantService{

    private Logger logger = LoggerFactory.getLogger(UserBonusGrantServiceImpl.class);
    ...
    @Override
    public void referrerTakeAll(Integer selfUserId, Integer referrerUserId,
                                Integer subAccountId, Double amount) {
    	try {
    		//判断推荐人是否存在
    ...
    @Override
    public void eachTakePart(Integer selfUserId, Integer referrerUserId,
                             Integer subAccountId, Double amount) {
    	try {
    		//判断推荐人是否存在
            BsUser recommend = userMapper.selectByPrimaryKey(referrerUserId);
    ...
    public String getBonusGrantTypeByUserId(Integer userId) {
        ...
        //之后的用户
            ...
            if(null != user.getAgentId() && (31 == user.getAgentId() || 34 == user.getAgentId()))
     ...
     */
    @Override
    public void selfTakePart(Integer selfUserId, Integer subAccountId, Double amount) {
    	try {
    		//查询产品子账户
            BsSubAccount productAccount = subAccountMapper.selectByPrimaryKey(subAccountId);
            //查询理财产品信息
    ...
    @Override
    public void entrustEachTakePart(Integer selfUserId, Integer referrerUserId, Integer subAccountId, Double amount) {
        try {
            ...
            if(recommend != null && recommend.getStatus() == Constants.USER_STATUS_1) {
                ...
                //查询理财产品信息
                BsProduct product = productMapper.selectByPrimaryKey(productAccount.getProductId());
                String referrerKey = Constants.BONUS_RATE_4_REFERRER_INSTALLMENT;
                String selfKey = Constants.BONUS_RATE_4_SELF_INSTALLMENT;
                //获得奖励金利率
                BsSysConfig referrerConfig = sysConfigMapper.selectByPrimaryKey(referrerKey);
                BsSysConfig selfConfig = sysConfigMapper.selectByPrimaryKey(selfKey);
    ...
    @Override
    public void entrustSelfTakePart(Integer selfUserId, Integer subAccountId, Double amount) {
        try {
