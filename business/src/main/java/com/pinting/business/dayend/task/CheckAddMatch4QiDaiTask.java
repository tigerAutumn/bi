 ...
 */
@Service
public class CheckAddMatch4QiDaiTask {
	private Logger log = LoggerFactory.getLogger(CheckAddMatch4QiDaiTask.class);
	@Autowired
	...
	String propertySymbol_7Dai = Constants.PRODUCT_PROPERTY_SYMBOL_7_DAI;
	...
	private void matchOver() {
		log.info("===============7贷【理财到期，匹配关系结束】开始=================");
		//查询 尚有匹配数据、理财已到期、债权非“已全部还款”、债权记录的资金源为7贷 的匹配数据列表
		final List<BsMatch> matches = bsMatchService.findOverMatches(propertySymbol_7Dai);
		if(!CollectionUtils.isEmpty(matches)){
				...
				protected void doInTransactionWithoutResult(TransactionStatus status) {
					for (final BsMatch bsMatch : matches) {
						...
						changeExample.createCriteria().andDealStatusEqualTo(Constants.MATCH_AMOUNT_DEAL_STATUS_N)
							...
							.andPropertySymbolEqualTo(propertySymbol_7Dai);
						...
						if(CollectionUtils.isEmpty(changes)){
							log.info("===============7贷【理财到期，匹配关系结束】债权编号【"+bsMatch.getLoanRelativeId()+"】无还款金额变动，直接回退金额至债权数据表=================");
						...
						//有还款金额变动，回退金额需比较处理
							...
							if(overMatchAmount < totalRepayAmount){
								log.info("===============7贷【理财到期，匹配关系结束】债权编号【"+bsMatch.getLoanRelativeId()+"】匹配结束金额<还款变动金额，还款变动金额变少=================");
							...
							//匹配结束金额>=还款变动金额
								...
								log.info("===============7贷【理财到期，匹配关系结束】债权编号【"+bsMatch.getLoanRelativeId()+"】匹配结束金额>=还款变动金额，还款变动记录作已处理操作，并剩余部分金额回退=================");
								change.setUpdateTime(new Date());
			...
			log.info("===============7贷【理财到期，匹配关系结束】无理财到期的匹配数据列表=================");
		}
		log.info("===============7贷【理财到期，匹配关系结束】结束=================");
	}
	...
	private void repayAmountChange(){
		...
		if(!CollectionUtils.isEmpty(amountChanges)){
			log.info("===============7贷【还款金额匹配】新增["+amountChanges.size()+"]条金额变动需匹配处理=================");
			for (BsLoanRelativeAmountChange amountChange : amountChanges) {
				...
				//在匹配表中有关联，遍历B
					...
					if(totalRepayAmount > 0){
						log.warn("===============7贷【还款金额匹配】还款编号["+amountChange.getId()+"]还款金额未完全匹配=================");
					}
					//遍历B结束后，修改a为已处理
					BsLoanRelativeAmountChange updateAmountChange = new BsLoanRelativeAmountChange();
					updateAmountChange.setId(amountChange.getId());
	...
	private void newAddLoanMatch() {
		...
		Integer oldCount = bsSubAccountMapper.countNewSubAccountREGList(interestBeginLimitDate, today, propertySymbol_7Dai);
		if(oldCount >0){
			Integer pageSize = 100;
			Integer c = oldCount/pageSize + (oldCount%pageSize> 0?1:0);
			for(int i=0;i<c;i++){
				//获取起息日在interestBeginLimitDate之后，且status为2-投资中, 债权关系匹配且钱报用户的投资在前
				List<BsSubAccount> list = bsSubAccountMapper.getNewSubAccountREGList(i*pageSize,pageSize,interestBeginLimitDate, today, propertySymbol_7Dai);
				if(list != null && list.size()>0){
					log.info("===============定时任务【7贷-债权关系匹配-新】"+interestBeginLimitDate+"之后status为2,获取列表："+list.size()+"=================");
	...
	private void dayfLoanSubAccountMatch(List<BsSubAccount> accountList) {
		...
		log.info("==========定时任务【7贷-债权关系匹配-7贷-全量】循环匹配开始==========");
	...
	private void matchNeedAmount(Double needMatchAmount, String buyTimeStart, String buyTimeEnd, BsSubAccount bsSubAccount){
		if(needMatchAmount > 0){
			...
			loanList = bsLoanRelativeRecordMapper.selectByBuyTime(buyTimeStart, buyTimeEnd, propertySymbol_7Dai); //获取符合匹配条件的债权数据
			...
			if(loanList != null && loanList.size()>0){
				...
				log.warn("==========定时任务【债权关系匹配-7贷-全量】编号"+bsSubAccount.getId()+"无债权数据列表匹配或未匹配完全==========");
				String type = "定时任务【债权关系匹配-7贷-全量】";
				String detail = "定时任务【债权关系匹配-7贷-全量】编号"+bsSubAccount.getId()+"无债权数据列表匹配或未匹配完全";
	...
	private Double getMatchAmount(Double leftAmount, String note, Double needMatchAmount) {
		...
		if(StringUtils.isNotBlank(note) && bsAgentViewConfigService.isQianbao(Integer.parseInt(note))){
			...
			if(needMatchAmount <= 20000){
				...
				canMatchAmount = (double)((rand.nextInt(200)+1)*100+20000); //(5000,20000]
				if(MoneyUtil.subtract(canMatchAmount,needMatchAmount).doubleValue() > 0){
					canMatchAmount = needMatchAmount; 
				}
			}else{
				canMatchAmount = (double)((rand.nextInt(100)+1)*100+40000); //(5000,20000]
				if(MoneyUtil.subtract(canMatchAmount,needMatchAmount).doubleValue() > 0){
					canMatchAmount = needMatchAmount; 
				}
			}
	...
	private void match(final Double amount, final Integer userId, final Integer subAccountId,
		...
		transactionTemplate.execute(new TransactionCallbackWithoutResult(){
			...
			protected void doInTransactionWithoutResult(TransactionStatus status) {
				...
				bsMatch.setPropertySymbol(propertySymbol_7Dai);
	...
	private void loanPatch() {
		try {
			...
			List<Map<String, Object>> noMatchMaps = bsSubAccountMapper.getNoMatchList(interestBeginDate, propertySymbol_7Dai);
			if(!noMatchMaps.isEmpty()){
				log.info("==========定时任务【7贷-债权关系匹配-新】统计日债权匹配差额超100：共" + noMatchMaps.size() + "天==========");
				...
				for(Map<String, Object> map : noMatchMaps){
					...
					//List<Map<String, Object>> needMatch4DayMaps = bsSubAccountMapper.getNeedMatchList(time, matchedPercent,0.3, propertySymbol_7Dai);
					Integer needMatch4DayCount = bsSubAccountMapper.getNeedMatchCount(time, matchedPercent,0.3, propertySymbol_7Dai);
					if(needMatch4DayCount == 0){
						log.info("==========定时任务【7贷-债权关系匹配-新】" + DateUtil.formatYYYYMMDD(time) + "债权无需再次匹配==========");
						continue;
					}else{
						log.info("==========定时任务【7贷-债权关系匹配-新】" + DateUtil.formatYYYYMMDD(time) + "获得" +needMatch4DayCount+ "笔债权需再次匹配==========");
					}
					
					//Double sumBalance = (Double) map.get("sumBalance"); //未匹配对应的投资总额
					Double sumNoMacthAmount = (Double) map.get("sumAmount"); //未匹配的总金额
					Double sumInvestBalance = bsSubAccountMapper.sumInvestBalanceByDate(time, propertySymbol_7Dai); //某日当前投资总额
					...
					if(percent < 1){
						...
						Integer amount4Day = MoneyUtil.multiply(percent, MoneyUtil.subtract(sumInvestBalance, sumNoMacthAmount).doubleValue()).intValue();
						// Integer amount4Day = (int)(percent*(sumInvestBalance-sumNoMacthAmount)); //某日待转让的匹配金额
						
						//根据起息时间查询已经完全匹配的和匹配金额超过一定比例的列表且已匹配金额超过一定值
						List<Map<String, Object>> matchedMaps = bsSubAccountMapper.getMatchedList(time, matchedPercent, topBalance, propertySymbol_7Dai);
						if(!matchedMaps.isEmpty()){
							log.info("==========定时任务【7贷-债权关系匹配-新】" + DateUtil.formatYYYYMMDD(time) + "查到"+matchedMaps.size()+"笔理财可转让债权匹配==========");
							...
							for(Map<String, Object> matched : matchedMaps){
								...
								if(amount4Day4Sub == 0){
									log.warn("==========定时任务【7贷-债权关系匹配-新】子账户编号" + matched.get("id") + "待转让的匹配金额计算为0，无法转让匹配关系==========");
									continue;
								}
								...
								if(amount4Day == 0){
									log.warn("==========定时任务【7贷-债权关系匹配-新】" + DateUtil.formatYYYYMMDD(time) + "待转让的匹配金额为0，无法转让匹配关系==========");
									break;
					...
					if(mList != null && mList.size() > 0){
						log.info("==========定时任务【7贷-债权关系匹配-新】" + DateUtil.formatYYYYMMDD(time) + "获得"+mList.size()+"笔匹配明细 待执行关系转让==========");
						//有可以分配的债权
						//查询需要获得债权的列表；
						for (BsMatchVO match : mList) {
							...
							List<Map<String, Object>> needMatchMaps = bsSubAccountMapper.getNeedMatchList(time, matchedPercent,0.3, propertySymbol_7Dai);
