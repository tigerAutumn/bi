 ...
 */
public class WxUserFacade {
	private static final Logger log = LoggerFactory.getLogger(WxUserFacade.class);
	@Autowired
	private WxUserService wxUserService;
	@Autowired
	private BsWxInfoMapper bsWxInfoMapper;

	public void isExist(ReqMsg_WxUser_isExist req, ResMsg_WxUser_isExist res) {
		int wxUserId = 0;
	...
	public void regist(ReqMsg_WxUser_Regist req, ResMsg_WxUser_Regist res) {
	...
	//关注用户
	public void subscribe(ReqMsg_WxUser_Subscribe req, ResMsg_WxUser_Subscribe res) {
	...
	// 公众号渠道二维码用户关注
	public void subscribeWxAgent(ReqMsg_WxUser_SubscribeWxAgent req, ResMsg_WxUser_SubscribeWxAgent res) {
		String openId = req.getOpenId();
		if(StringUtil.isNotEmpty(openId)){
			BsWxInfoExample example = new BsWxInfoExample();
			example.createCriteria().andOpenIdEqualTo(openId);
			...
			if(list != null && list.size() > 0) {
				...
				if(StringUtil.isNotEmpty(userMessage)) {
					try {
						...
						info.setFollowTime(new Date());
						info.setIsFollow("FOLLOW");
						info.setWxAgentId(req.getWxAgentId());
						bsWxInfoMapper.insertSelective(info);
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	//取消关注用户
	public void unSubscribe(ReqMsg_WxUser_UnSubscribe req, ResMsg_WxUser_UnSubscribe res) {
		String openId = req.getOpenId();
		if(StringUtil.isNotEmpty(openId)){
			BsWxInfoExample example = new BsWxInfoExample();
			example.createCriteria().andOpenIdEqualTo(openId);
