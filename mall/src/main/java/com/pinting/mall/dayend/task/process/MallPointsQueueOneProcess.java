 ...
 */
public class MallPointsQueueOneProcess implements Runnable{

	private static JedisClientDaoSupport jsClientDaoSupport = JedisClientDaoSupport.getInstance(Constants.REDIS_SUBSYS_BUSINESS);
	
	private Logger log = LoggerFactory.getLogger(MallPointsQueueOneProcess.class);
	...
	public void run() {
		...
		try {
			if( mallPointsDTO != null ) {
                //调用积分发放业务
                if(MallRuleEnum.MALL_REGISTER.getCode().equals(mallPointsDTO.getTransType())) {
                	mallAccountService.registerPointsIn(mallPointsDTO);
                } else if(MallRuleEnum.MALL_OPEN_DEPOSIT.getCode().equals(mallPointsDTO.getTransType())) {
                	mallAccountService.openDepositePointsIn(mallPointsDTO);
                } else if(MallRuleEnum.MALL_FINISH_RISK_ASSESSMENT.getCode().equals(mallPointsDTO.getTransType())) {
                	mallAccountService.riskAssessmentPointsIn(mallPointsDTO);
                }
            } else {
