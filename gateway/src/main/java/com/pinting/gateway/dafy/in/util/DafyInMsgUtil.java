...
//	private final static String dafyInDESKey = "dafy77777770dafy.com1,.*";
	private static Logger log = LoggerFactory.getLogger(DafyInMsgUtil.class);
	...
	private static String[] transUnits = new String[]{
		...
		"cutRepayConfirm,token,transCode,requestTime,clientKey,orderNo,overdueFlag,userId,name,idCard,mobile,bankCard,bankCode,totalAmount,userSignNo,payIP,loans",//代扣还款
...
//					Class.forName(modelClassName.toString()));
	 ...
	 * @return
	 */
	public static <T> String dafyLoanPackageMsgHash(T t, String transCode) {
		// 组装hash字段
		List<String> unitList = transUnitMap.get(transCode);
		if(unitList == null){
			return null;
...
//		System.out.println(MD5Util.encryptMD5(hash));
		...
		System.out.println("每日可借额度查询回款:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(dailyAvailableAmountLimitMessage), "utf-8"));
		...
		System.out.println("放款:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(applyLoanMessage), "utf-8"));
		...
		System.out.println("放款结果查询:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(queryLoanResultMessage), "utf-8"));
	    ...
	    System.out.println("账单（还款计划推送）:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(pushBillMessage), "utf-8"));
		...
		System.out.println("借款协议签章结果查询:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(querySignResultMessage), "utf-8"));
	    ...
	    System.out.println("主动还款预下单:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(activeRepayPreMessage), "utf-8"));
	    ...
	    System.out.println("主动还款确认下单:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(activeRepayConfirmMessage), "utf-8"));
		...
		System.out.println("补账完成通知:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(fillFinishMessage), "utf-8"));
	    ...
	    System.out.println("代偿通知:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(lateRepayMessage), "utf-8"));
		...
		System.out.println("还款结果查询:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(queryRepayResultMessage), "utf-8"));
		...
		System.out.println("代扣还款:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(cutRepayConfirmMessage), "utf-8"));
		...
		System.out.println("协议下载地址查询:"+URLEncoder.encode(new DESUtil(dafyInDESKey).encryptStr(agreementNoticeMessage), "utf-8"));
		*/

		
		QueryDailyAmountResModel resModel = new QueryDailyAmountResModel();
		resModel.setAmount(null);
		String message;
		try {
			message = JacksonUtil.bean2Json(resModel);
			System.out.println(message);
			
			System.out.println(JsonLibUtil.bean2Json(resModel));
		} catch (IOException e) {
