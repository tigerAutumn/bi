 ...
 * @version $Id: FtpClientUtil.java, v 0.1 2015-8-31 下午8:39:35 BabyShark Exp $
 */
public class FtpClientUtil {
    ...
    public FtpClientUtil(String username, String password, String ftpHostName, int port) {
        ...
        try {
            ...
            if (isDir(ftpFileName)) {
                ...
                for (int i = 0; i < names.length; i++) {
                    logger.info(names[i] + "^^^^^^^^^^^^^^");
                    if (isDir(names[i])) {
                        downFileOrDir(ftpFileName + '/' + names[i], localDir + File.separator
                                                                    + names[i]);
    ...
    public static void main(String[] args) {
        String ftpHost = "119.254.84.6";//GlobEnvUtil.get("19pay.ftp.host");
        String ftpPort = "21";//GlobEnvUtil.get("19pay.ftp.port");
        String username = "90003243";//GlobEnvUtil.get("19pay.ftp.username");
        String password = "Le~DxtjI";//GlobEnvUtil.get("19pay.ftp.password");
        FtpClientUtil ftpUtil = new FtpClientUtil(username, password, ftpHost,
            Integer.valueOf(ftpPort));
        ftpUtil.download("/20150830.csv", "f:/");
    }
