...
/**
 * 融宝 统一  Business Hessian接入  接口实现
 * @Project: gateway
 * @Title: ReapalGatewayHessianServiceImpl.java
 * @author dingpf
 * @date 2015-2-10 下午2:17:40
 * @Copyright: 2015 BiGangWan.com Inc. All rights reserved.
 */
public class ReapalGatewayHessianServiceImpl extends HessianBaseService implements HessianService , ApplicationContextAware{
	
	protected static ApplicationContext applicationContext;
	private static ValidatorFactory validatorFactory = Validation.buildDefaultValidatorFactory(); 
	
	private Logger log = LoggerFactory.getLogger(ReapalGatewayHessianServiceImpl.class);
	...
	public ResMsg handleMsg(ReqMsg reqMsg) {
		...
		try {
			...
			String resClassName = Constants.BUSINESS_REAPAL_MESSAGE_RESMSG_PRE + facadeName + "_" + actionName;
			...
			for (Method method : bsFacade.getClass().getDeclaredMethods()) {
				if(method.getName().equals(methodName)
					...
					try {
						...
						}else if(cause instanceof PTException){
							throw new PTException(((PTException)cause).getErrCode(), ((PTException)cause).getErrMessage(), cause);
						}else{
	...
	public void setApplicationContext(ApplicationContext applicationContext)throws BeansException {
		ReapalGatewayHessianServiceImpl.applicationContext = applicationContext;
