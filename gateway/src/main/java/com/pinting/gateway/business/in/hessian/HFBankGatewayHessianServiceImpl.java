 ...
 * Author:      cyb
 * Date:        2017/4/3
 * Description: 统一  Business Hessian接入  接口实现
 */
public class HFBankGatewayHessianServiceImpl extends HessianBaseService implements HessianService, ApplicationContextAware {

    protected static ApplicationContext applicationContext;
    private static ValidatorFactory validatorFactory = Validation.buildDefaultValidatorFactory();

    private Logger log = LoggerFactory.getLogger(HFBankGatewayHessianServiceImpl.class);
    ...
    public ResMsg handleMsg(ReqMsg reqMsg) {
        ...
        try {
            ...
            String resClassName = Constants.BUSINESS_HF_BANK_MESSAGE_RESMSG_PRE + facadeName + "_" + actionName;
                ...
                if (method.getName().equals(methodName)
                    ...
                    try {
                        ...
                        if (cause instanceof PTMessageException) {
                            String errorCode = ((PTMessageException) cause).getErrCode();
                            String errorMsg = ((PTMessageException) cause).getErrMessage();

                            if ("920001".equals(errorCode)) {

                                log.info("======交易失败[" + errorMsg + "]======");
                                //截取
                                String[] resMessage = errorMsg.split("：");
                                String[] resMessages = resMessage[resMessage.length - 1].split(",");
                                if (resMessages.length == 2) {
                                    String thirdReturnCode = resMessages[0];
                                    String thirdReturnMsg = resMessages[1];

                                    QuickPayRespCode code = QuickPayRespCode.find(thirdReturnCode);

                                    log.info("third: code == "+thirdReturnCode + ", msg == "+thirdReturnMsg);
                                    errorMsg =  code != null ? code.getDescription() : thirdReturnMsg;
                                    HashMap<String, Object> extendMap = new HashMap<String, Object>();
                                    extendMap.put("thirdReturnCode", thirdReturnCode);
                                    resMsg.setExtendMap(extendMap);
                                }
                            }
                            log.info("gateway hessian: errorCode == "+ errorCode + ", errorMsg == "+errorMsg);
                            throw new PTException(errorCode, errorMsg, cause);
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        HFBankGatewayHessianServiceImpl.applicationContext = applicationContext;
