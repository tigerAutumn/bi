 ...
 */
public class ZANInMsgUtil {
	private final static String ZANINDESKEY = "zankjdfg9809564352134Oopinting.com1,.*";
	private static Logger log = LoggerFactory.getLogger(ZANInMsgUtil.class);
	...
	public static BaseReqModel parseMsg(String ciphertext) {
		...
		try {
			// DES解密，获得明文
			String decryptData = new DESUtil(ZANINDESKEY).decryptStr(ciphertext);
...
//					Class.forName(modelClassName.toString()));
	...
	public static String packageMsg(BaseResModel resModel) {
		...
		try {
			...
			// 对json进行DES加密
			ciphertext = new DESUtil(ZANINDESKEY).encryptStr(message);
