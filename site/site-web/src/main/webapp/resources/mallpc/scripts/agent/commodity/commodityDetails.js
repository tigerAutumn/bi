var global={
		...
		go_buy:'/gen178/regular/list?agentViewFlag='+$("#agentViewFlag").val(),
		off_btn:true,
}
$(function(){
	//X关掉弹窗
	...
	$(".detailsTitleBtn").click(function(){
		$.ajax({
			url: global.base_url + '/pc_a/mallCommodity/exchangeCheck?commodityId='+commodityId,
			type: 'get',
			dataType:"json",
...
function go_exchange(){
	...
	$.ajax({
		url: global.base_url + '/pc_a/mallCommodity/exchangeCommodity',
		type: 'post',
		dataType:"json",
		data:{
			id:$("#id").val(),
		...
		success:function(data){
			if(data.resCode == "000000" && data.exchangeResult == true) {				
				setTimeout(function(){
					location.href=global.base_url+'/pc_a/mallCommodity/exchangeSuccPage?agentViewFlag='+$("#agentViewFlag").val()
				},500);																			
			}else{
				drawToast(data.resMsg);
				setTimeout(function(){
