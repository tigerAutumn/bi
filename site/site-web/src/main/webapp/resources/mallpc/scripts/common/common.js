...
function user_out(obj) {
	location.href = $('#APP_ROOT_PATH_GEN').val() + '/gen2.0/user/login/out';
}
// 我的资产
function my_assets(obj) {
	location.href = $('#APP_ROOT_PATH_GEN').val() + '/gen2.0/assets/assets';
}
$(function() {
    var url = $('#APP_ROOT_PATH_GEN').val() + '/gen2.0/get_nick_cookie';
    $.ajax({
        type: "post",
        url: url,
        data: "",
        success: function (data) {
            var nullNick = '<li>hi,欢迎来到币港湾</li> <li><a href="' + $("#APP_ROOT_PATH_GEN").val() + "/gen2.0/user/login/index" + '">登录</a></li><li><a href="' + $("#APP_ROOT_PATH_GEN").val() + "/gen2.0/user/register_first_new_index" + '">注册</a></li>';
            ...
            /*var win_p_out= '<p class='+'"window_p_3"'+'><a href="'+$("#APP_ROOT_PATH").val()+"/gen2.0/user/register_first_new_index"+'">免费注册</a></p><p class='+'"window_p_2"'+'><a href="'+$("#APP_ROOT_PATH").val()+"/gen2.0/user/login/index"+'">登录</a></p>';
             var win_p_login = '<p class='+'"window_p_1"'+'><a href="'+$("#APP_ROOT_PATH").val()+"/gen2.0/assets/assets"+'">进入我的账户</a></p>';
...
function closeDrawDiv() {
    ...
    function drawAlertNew(message, title, commitMessage, backUrl, commitUrl) {
        ...
        var url = $('#APP_ROOT_PATH_GEN').val() + '/gen2.0/static/alert_new';
        $.ajax({
            type: "GET",
            url: url,
            data: "",
    ...
    function drawAlertConfirmNew(message, title, commitMessage, backMessage, commitUrl, backUrl) {
        ...
        var url = $('#APP_ROOT_PATH_GEN').val() + '/gen2.0/static/alert_confirm_new';
        $.ajax({
            type: "GET",
            url: url,
            data: "",
...
var _global = (function() {
    ...
    return {
        ...
        _go_url: function(go_url, params) {
            // 默认登录页
            var url = _base_url() + '/gen2.0/user/login/index';
            if(go_url) {
