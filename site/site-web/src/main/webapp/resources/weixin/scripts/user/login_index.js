$(function() {
	...
	var global = {
		...
		login_url: '/weixin/user/login',
		active_url: '/weixin/activity/weChatLuckyTurning',
			...
			success: function (data) {
				...
				if (data.resCode == '000') {
					if(data.burl != null && data.burl != '') {
						url = global.base_url + data.burl;
					} else {
						url = global.base_url + global.active_url;
					}
	...
	$('.login_register').on('click',function () {
		window.location.href= global.base_url + '/weixin/user/register_first_new_index' + '?burl=' + global.burl
	});
	$('.login_forget').on('click',function () {
		window.location.href= global.base_url + '/weixin/user/forget_password_first' + '?burl=' + global.burl
	});
	//禁止输入空格
