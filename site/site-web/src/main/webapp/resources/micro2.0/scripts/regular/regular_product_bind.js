$(function(){
	 ...
	 */
	function goTopUp() {
		sessionStorage.entry = 'buy';
		sessionStorage.id = $('#id').val();
		sessionStorage.entry_time = new Date();
		if(global.qianbao) {
			location.href = global.top_up_url + "?qianbao=qianbao&agentViewFlag="+$('#agentViewFlag').val();
		} else {
			location.href = global.top_up_url;
		}
	}
	/**
	 * 监听输入框输入做出相应操作的函数
	...
	function confirmOrder() {
		...
		$.ajax({
			...
			success:function(data){
				closeDrawDiv();
				//隐藏弹窗
                $("#paylog").addClass('alert_hide').removeClass("alert_open");

				if(data.errorCode == CONSTANTS.CODE.SUCCESS_CODE_000 || data.errorCode == CONSTANTS.CODE.SUCCESS_CODE_000000) {
					var url = global.result_url + '?code='+data.errorCode+"&msg="+data.errorMsg+"&productId="+$("#id").val().trim();
					if(global.qianbao) {
						url += "&qianbao=qianbao&agentViewFlag="+$('#agentViewFlag').val();
					}
					location.href = url;
				} else if(data.errorCode == CONSTANTS.CODE.NOT_BIND_CARD) {
					location.href = global.bind_card_url;
				} else {
    ...
    function reloadPage() {
        ...
        form.append($("#id").clone(true));
        form.append($("#term").clone(true));
        form.append($("#rate").clone(true));
        form.append($("#name").clone(true));
        form.append($("#minInvestAmount").clone(true));
        form.append($("#userType").clone(true));
        if (global.qianbao) {
...
//风险提示点击事件
function clickriskWarningInfo() {
	var url = $("#APP_ROOT_PATH").val()+'/micro2.0/agreement/riskWarningInfo';
	$("#submitForm").attr('action', url);
	$("#submitForm").submit();
}

//自动出借计划协议点击事件
function clickEntrustBuyAgreement() {
	var url = $("#APP_ROOT_PATH").val()+'/micro2.0/agreement/entrustBuyAgreementApp';
	$("#submitForm").attr('action', url);
	$("#submitForm").submit();
}

//授权委托书点击事件
function clickAgreementFinancial() {
	var url = $("#APP_ROOT_PATH").val()+'/micro2.0/agreement/financial';
	$("#submitForm").attr('action', url);
	$("#submitForm").submit();
}
