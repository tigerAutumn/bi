				...
				function(data){
					...
					if(data.resCode=='000'){
						...
						if(qianbao && qianbao == 'qianbao') {
							location.href=$("#APP_ROOT_PATH").val()+"/micro2.0/user/forget_password_second?mobile="+$('#mobile').val()+"&qianbao=qianbao&agentViewFlag="+$('#agentViewFlag').val()+"&verCode="+verCode;
						} else {
							location.href=$("#APP_ROOT_PATH").val()+"/micro2.0/user/forget_password_second?mobile="+$('#mobile').val()+"&verCode="+verCode;
						}
					} else {
						change();
