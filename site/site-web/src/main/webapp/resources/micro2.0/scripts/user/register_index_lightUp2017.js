    ...
    function postData(currentForm, formType){
    	...
    	$.ajax({
    		...
    		success: function(data) {
    			if(data.bsCode=='000'){
					//注册成功-跳转点亮活动页
    				var url =  $("#APP_ROOT_PATH").val()+ "/micro2.0/activity/lightUpIndex";
					location.href= url;
				}else{
					if(data.bsMsg){
...
function go_login(){
	//登陆成功跳转到活动页
	var burl = "/micro2.0/activity/lightUpIndex";
	location.href = $("#APP_ROOT_PATH").val() + "/micro2.0/user/login/index?burl=" + burl;
}
