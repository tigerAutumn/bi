$(function(){
	...
	function mintuesChange(codeBtn){
		return function(){
			...
			var count = interval;
			if(!count || count<=0){
				$(codeBtn).html('重发').removeAttr('disabled').attr('count','60');
			...
			$(codeBtn).html((--localStorage.interval)+'s').attr('count',count);
		};
	}
	...
	function sendCodeSuccOption() {
		...
		if(interval && interval>0) {
			$("#sendCode").off('click');
			var count = interval;
			$('#sendCode').attr('disabled','disabled').html((count)+'s').attr('count',count);
    ...
    function postDataRegister(){
		...
		var url =  $("#APP_ROOT_PATH").val()+ "/micro2.0/regular/firstBuySubmitReapalQuickCMB";
    	$.ajax({
    		url: url,
    		type: "post",
    		data:{
    		...
    		success: function(data) {
    			...
    			localStorage.removeItem('interval');
    			if(data.resCode=='000'){
					var url =  $("#APP_ROOT_PATH").val()+ "/micro2.0/regular/regular_success";
					location.href= url;
				} else{
					var url =  $("#APP_ROOT_PATH").val()+ "/micro2.0/regular/reapal_quick_cmb_fail?reapalMessage="+data.resMsg+"&qianbao="+qianbao+"&agentViewFlag="+$('#agentViewFlag').val();
					location.href= url;
				}
			},
			error: function(data) {
	...
	function resendCode(){
		var orderNo = $.trim($("#orderNo").val());
		var url =  $("#APP_ROOT_PATH").val()+ "/regular/reapal/resendCode";  //重发验证码
    	$.ajax({
    		url: url,
    		type: "post",
    		data:{
