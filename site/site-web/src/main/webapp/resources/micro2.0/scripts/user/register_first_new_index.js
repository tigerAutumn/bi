		...
		$.post($("#APP_ROOT_PATH").val()+"/micro2.0/identity/registerMobilecode",
			...
			function(data){
				if(data.resCode=='000'){
					...
					location.href=$("#APP_ROOT_PATH").val()+"/micro2.0/user/register_second_index?mobile="+$('#mobile').val() +"&verCode="+ $('#verCode').val() +"&qianbao="+$("#qianbao").val()+"&recommendId="+$("#recommendId").val()+"&agentViewFlag="+$('#agentViewFlag').val()+'&flagForQD='+$('#flagForQD').val();
