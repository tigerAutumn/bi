...
var formValidate = function() {
	return {
		...
		// 表单验证依赖函数 开始
		errorShow:function (tempField, showFlag){
			//$(tempField).focus();
			$(tempField).addClass('fieldHasError');
			//$(tempField + 'FieldError').fadeIn(300);
			var message = $(tempField + 'FieldError').text();
			var errSpan = $(tempField + 'FieldError').find("span");
			if(errSpan.length>1){
				if(showFlag == "NOT_NULL"){
					message = $(errSpan).eq(0).text();
				}else if(showFlag == "REG_ERROR"){
					message = $(errSpan).eq(1).text();
				}
			}
			drawToast(message);
			...
			for(var i=0; i<fieldArray.length; i++){
				...
				if($(field).hasClass('requiredField')){
					if($(field).val() == '' || $(field).val() == $(field).attr('data-placeholder')){
						...
						formValidate.errorShow(tempField, "NOT_NULL");
						return formValidateFlag;
					};
				}
				//不为空 校验 结束
				...
				if($(field).hasClass('regExpField')){
					...
					if(formValidateFlag == false){
						formValidate.errorShow(tempField, "REG_ERROR");
						return formValidateFlag;
					}
				}
				//正则表达式校验结束
