$(function(){
	...
	$(".leftbg").on('click', function(){
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(isInLogin) {
					$.ajax({
						url: root + '/gen178/assets/assets_index',
						type: 'post',
						dataType: 'html',
						success: function(data) {
							subNavClick(obj);
					...
					location.href = root + '/gen178/assets/assets' + '?agentViewFlag=' + global.agentViewFlag;
				}
			}
		});
	});
	...
	$(".invest_manage").on('click', function(){
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(isInLogin) {
					$.ajax({
						url: root + '/gen178/assets/invest_manage',
						type: 'post',
						dataType: 'html',
						data : {
							type : "bgw"
	...
	$(".payment_plant").on("click",function () {
		...
		$.ajax({
			...
			success: function (data) {
				...
				if(isLnLogin){
					$.ajax({
						url: root + '/gen178/assets/payment_plant',
						type: 'post',
						dataType: 'html',
						data:{
							dateTime : dateTime,
	...
	$(".item_ft").on('click', function(){
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(isInLogin) {
					$.ajax({
						url: root + '/gen178/assets/trading_detail',
						type: 'post',
						dataType: 'html',
						success: function(data) {
							subNavClick(obj);
	...
	$(".recharge").on('click', function(){
		...
		if(isOpened == "NO_BIND_CARD" || isOpened == "FAILED_BIND_HF") {
			drawToast("请先开通存管");
			setTimeout(function() {
				location.href = root + '/gen178/bankcard/index?entry=top_up' + '&agentViewFlag=' + global.agentViewFlag;
			}, 2000);
			return false;
			...
			drawToast("请先激活存管");
			setTimeout(function() {
				location.href = root + "/gen178/bankcard/activate/index" + '?agentViewFlag=' + global.agentViewFlag;
			}, 2000);
			return false;
		} else if(riskStatus == "no"){
			drawToast("风险承受能力测评完成后可加入产品、充值、提现");
			setTimeout(function() {
				location.href = root + "/gen178/assets/questionnaire" + '?agentViewFlag=' + global.agentViewFlag;
				sessionStorage.clear();
			}, 2000);
			return false;
		} else if(riskStatus == "expire"){
			drawToast("您尚未进行风险承受能力测评，重新测评后可加入产品、充值、提现");
			setTimeout(function() {
				location.href = root + "/gen178/assets/questionnaire" + '?agentViewFlag=' + global.agentViewFlag;
				sessionStorage.clear();
			}, 2000);
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(isInLogin) {
					$.ajax({
						url: root+'/gen178/safe/checkPayPassword',
						type: 'post',
						dataType: 'json',
						success: function (data) {
							if (data.resCode == '000') {
								$.ajax({
									...
									success: function (data) {
										if(data.hfDepGuideInfo.isOpened == "WAIT_ACTIVATE") {
											...
											drawToast("请先激活存管");
											setTimeout(function() {
												location.href = $("#APP_ROOT_PATH_GEN").val() + "/gen178/bankcard/activate/index" + '?agentViewFlag=' + global.agentViewFlag;
											}, 2000);
											...
											// 有回款卡
											location.href = root + '/gen178/recharge/recharge_index' + '?agentViewFlag=' + global.agentViewFlag;
										}
									}
								});
							}
	...
	$(".withdraw").on('click', function(){
		...
		if(isOpened == "NO_BIND_CARD" || isOpened == "FAILED_BIND_HF") {
			drawToast("请先开通存管");
			setTimeout(function() {
				location.href = root + '/gen178/bankcard/index?entry=withdraw' + '&agentViewFlag=' + global.agentViewFlag;
			}, 2000);
			return false;
		} else if(isOpened == "WAIT_ACTIVATE") {
			// 待激活
				...
				if(isInLogin) {
					$.ajax({
						...
						success: function (data) {
							if (data.resCode == '000') {
								$.ajax({
									url: root + '/gen178/common/checkHFBankDepOpened',
									type: 'post',
									success: function (data) {
										if(data.hfDepGuideInfo.isOpened == "WAIT_ACTIVATE") {
											// 待激活
	...
	function postHtmlWithdraw(){
		location.href = root + "/gen178/withdraw/withdraw_deposit" + '?agentViewFlag=' + global.agentViewFlag;
	}
	// 设置交易密码
	...
	$(".set_pay_password_btn").click(function(){
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(isInLogin) {
					$.ajax({
						url: root + "/gen178/safe/safe_center",
						type: 'post',
						dataType: 'html',
						success: function(data){
							subNavClick(obj);
	...
	$(".questionnaire").on('click', function(){
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(isInLogin) {
					$.ajax({
						url: root + '/gen178/assets/questionnaireMore',
						type: 'post',
						dataType: 'html',
						success: function(data) {
							subNavClick(obj);
	...
	$(".my_red_packet").on('click', function(){
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(isInLogin) {
					$.ajax({
						url: root + '/gen178/redPacket/myRedPacket',
						type: 'post',
						dataType: 'html',
						success: function(data) {
							subNavClick(obj);
	...
	$(".my_bank_card").on('click', function(){
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(isInLogin) {
					$.ajax({
						url: root + '/gen178/wealth/bankcard_index',
						type: 'post',
						dataType: 'html',
						success: function(data) {
							subNavClick(obj);
		...
		if($(this).val().length>=6) {
			...
			$(".down_ok").on('click', function(){
				...
				if(payPassword == null || payPassword == '') {
					...
					$.ajax({
						url: root+'/gen178/safe/check_pay_password',
						data:{
						...
						success: function(data) {
							if(data.resCode == '000'){
								if(data.truePayPassword == true){	// 交易密码正确
									...
									$('.dialog_wrap').hide();
									if(data.failNums >= 6){
										drawAlerts('提示', data.toastMsg, "找回密码", $("#APP_ROOT_PATH_GEN").val()+"/gen178/safe/forget_pay_password" + '?agentViewFlag=' + global.agentViewFlag);
									} else {
										if((5-data.failNums) == 0) {
											drawAlerts('提示', "交易密码输入错误次数过多，请180分钟后再试，或尝试找回密码", "找回密码", $("#APP_ROOT_PATH_GEN").val()+"/gen178/safe/forget_pay_password" + '?agentViewFlag=' + global.agentViewFlag);
										} else {
	...
	function postHtml(){
		$.ajax({
			url: root+'/gen178/assets/return_path_index',
			type: 'post',
			dataType: 'html',
			success: function(data){
				closeDrawDiv();
	...
	if($("#withdraw").val && $("#withdraw").val()=='withdraw'){
		$(".withdraw").click();
	}

	if($("#recharge").val && $("#recharge").val()=='recharge'){
		location.href = root + '/gen178/recharge/recharge_index' + '?agentViewFlag=' + global.agentViewFlag;
	}
	...
	var selfbank = $("#selfbank").val();
	if(selfbank && selfbank == "selfbank") {
		$(".my_bank_card").click();
	}

	var accountOverview = $("#accountOverview").val();
	if(accountOverview && accountOverview == "accountOverview") {
		$(".leftbg").click();
	}

	var redPacket = $("#redPacket").val();
	if(redPacket && redPacket == "redPacket") {
		$(".my_red_packet").click();
	}

	var investManage = $("#investManage").val();
	if(investManage && investManage == "investManage") {
		$(".invest_manage").click();
	}

	var repayPlan = $("#repayPlan").val();
	if(repayPlan && repayPlan == "repayPlan") {
		$(".payment_plant").click();
	}
	...
	var securityCenter = $("#securityCenter").val();
	if(securityCenter && securityCenter == "securityCenter") {
		$(".safe_center").click();
	}
	...
	if(pageFlag == 'questionnaire') {
		$('.questionnaire').click();
	}
});
function tzXyiPrint(obj){
	window.open($("#APP_ROOT_PATH_GEN").val()+"/gen178/regular/buyagreement?"+$(obj).attr('data-datas'),'_blank','width=800,height=600,menubar=no,toolbar=no, location=no,directories=no,status=no,scrollbars=yes,resizable=yes');
}
function ccPzPrint(obj){
	window.open($("#APP_ROOT_PATH_GEN").val()+"/gen178/regular/positionVoucher?"+$(obj).attr('data-datas'),'_blank','width=800,height=600,menubar=no,toolbar=no, location=no,directories=no,status=no,scrollbars=yes,resizable=yes');
}
function matchPrint(obj){
	window.open($("#APP_ROOT_PATH_GEN").val()+"/gen178/match/myMatch?"+$(obj).attr('data-datas'),'_blank','width=1200,height=600,menubar=no,toolbar=no, location=no,directories=no,status=no,scrollbars=yes,resizable=yes');
}
//自动出借服务协议
function autoLoanPrint(obj){//autoLoanPrint(obj)   $(obj).attr('data-datas'),
	window.open($("#APP_ROOT_PATH_GEN").val()+"/gen178/agreement/entrustAgreement?"+$(obj).attr('data-datas'),'_blank','width=800,height=600,menubar=no,toolbar=no, location=no,directories=no,status=no,scrollbars=yes,resizable=yes');
}