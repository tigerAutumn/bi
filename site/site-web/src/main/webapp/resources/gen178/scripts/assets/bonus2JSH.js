...
var global = {
	...
	withdraw_url: $("#APP_ROOT_PATH_GEN").val() + '/gen178/bonus/withdraw',					        // 提现链接
	...
	$.ajax({
		...
		success: function (data) {
			if (data.hfDepGuideInfo.isOpened == "WAIT_ACTIVATE") {
				...
				drawToast("请先激活存管");
				setTimeout(function () {
					location.href = $("#APP_ROOT_PATH_GEN").val() + "/gen178/bankcard/activate/index";
				}, 2000);
			} else if (data.hfDepGuideInfo.isOpened == "NO_BIND_CARD" || data.hfDepGuideInfo.isOpened == "FAILED_BIND_HF") {
				// 未绑卡 ||  恒丰批量开户失败
				closeDrawDiv();
				location.href = $("#APP_ROOT_PATH_GEN").val() + '/gen178/bankcard/index?entry=invite';
			...
			success: function(data) {
				if(data.resCode == '000'){
					if(!data.truePayPassword) {
						closeDrawDiv();
						if(data.failNums >= 6 || (5-data.failNums) == 0 ){
