$(function () {
	...
	var global = {
		...
		pre_balance_buy_url: $("#APP_ROOT_PATH_GEN").val() + "/gen178/identity/mobilecode",					// 余额购买预下单链接
		balance_buy_url: $("#APP_ROOT_PATH_GEN").val() + "/gen178/regular/balance_buy",						// 余额购买链接
		result_url: $("#APP_ROOT_PATH_GEN").val() + "/gen178/regular/regular_success",						// 结果页面链接
		top_up_url: $("#APP_ROOT_PATH_GEN").val() + "/gen178/recharge/recharge",							// 充值页面链接
		bind_card_url: $("#APP_ROOT_PATH_GEN").val() + "/gen178/bankcard/index?entry=buy&productId="+$("#id").val()+"&amount="+$("#amount").val(),
		time: 60,																							// 用于验证码倒计时
			...
			success: function(data) {
				if (data.resCode == '000' || data.resCode == '910005') {
					...
					if(data.resCode == '931014'){
						...
						setTimeout(function() {
							location.href = global.root_path_url + "/gen178/assets/questionnaire" + '?agentViewFlag=' + global.agentViewFlag;
	...
	$(".go_recharge").on('click',function () {
		$.ajax({
			url: global.root_path_url + '/gen178/common/checkHFBankDepOpened',
			type: 'post',
			success: function (data) {
				if(data.hfDepGuideInfo.riskStatus == "no") {
					drawToast("风险承受能力测评完成后可加入产品、充值、提现");
					setTimeout(function() {
						location.href = global.root_path_url + "/gen178/assets/questionnaire";
						sessionStorage.clear();
					}, 2000);
					return false;
					...
					location.href = global.root_path_url + '/gen178/recharge/recharge_index';
				}
			}
		});
	});
