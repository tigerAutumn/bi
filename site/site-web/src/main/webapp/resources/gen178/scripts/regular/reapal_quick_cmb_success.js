$(function(){
    ...
    function postDataRegister(){
		...
		var url =  $("#APP_ROOT_PATH_GEN").val()+ "/gen178/regular/firstBuySubmitReapalQuickCMB";
    	$.ajax({
    		url: url,
    		type: "post",
    		data:{
    		...
    		success: function(data) {
    			...
    			localStorage.removeItem('interval');
    			if(data.resCode=='000'){
					var url =  $("#APP_ROOT_PATH_GEN").val()+ "/gen178/regular/regular_success?agentViewFlag="+agentViewFlag;
					location.href= url;
				} else{
					var url =  $("#APP_ROOT_PATH_GEN").val()+ "/gen178/regular/reapal_quick_cmb_fail?reapalMessage="+data.resMsg+"&agentViewFlag="+agentViewFlag;
					location.href= url;
				}
			},
			error: function(data) {
