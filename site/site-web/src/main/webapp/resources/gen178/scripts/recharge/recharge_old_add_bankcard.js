$(function(){
    ...
    function comfirmPost(){
		var url = app_root_path + "/gen178/recharge/add_bankcard_submit";
    	...
    	$.ajax({
    		...
    		success: function(data) {
    			if(data.resCode == '000'){
    				location.href = app_root_path + "/gen178/recharge/recharge_success?agentViewFlag="+agentViewFlag;
				} else {
					if(data.resMsg){
						drawToast(data.resMsg);
					} else {
	...
	$(".z2_buut").on('click', function(){
		location.href = app_root_path + "/gen178/recharge/eBankSubmit?money="+$("#rechargeAmount").val()+"&agentViewFlag="+agentViewFlag;
	});
	// 网银确认支付结束
});
// 卡bin开始
