...
function plan_info(id) {
	...
	location.href = APP_ROOT_PATH + "/gen178/regular/index?id="+id + '&agentViewFlag=' + $('#agentViewFlag').val()
}

function go_product_list() {
	var APP_ROOT_PATH = $("#APP_ROOT_PATH_GEN").val();
	location.href = APP_ROOT_PATH + "/gen178/regular/list" + '?agentViewFlag=' + $('#agentViewFlag').val();
}

// 公告内容
function go_notice_detail(id) {
	var APP_ROOT_PATH = $("#APP_ROOT_PATH_GEN").val();
	location.href = APP_ROOT_PATH+'/gen178/platform/info/notice_detail/index?id='+id + '&agentViewFlag=' + $('#agentViewFlag').val()
}

// 公司动态
function go_company_detail(id) {
	var APP_ROOT_PATH = $("#APP_ROOT_PATH_GEN").val();
	location.href = APP_ROOT_PATH+'/gen178/platform/about/company_dynamic_detail/index?id='+id + '&agentViewFlag=' + $('#agentViewFlag').val()
}

// 运营报告
function go_operation_report() {
	var APP_ROOT_PATH = $("#APP_ROOT_PATH_GEN").val();
	location.href = APP_ROOT_PATH+'/gen178/platform/info/report/index' + '?agentViewFlag=' + $('#agentViewFlag').val()
}

// 媒体报道
function go_news(id){
	var APP_ROOT_PATH = $("#APP_ROOT_PATH_GEN").val();
	location.href = APP_ROOT_PATH+'/gen178/platform/about/media_detail/index?id=' + id + '&agentViewFlag=' + $('#agentViewFlag').val()
}
...
function clickPlatformData() {
	var APP_ROOT_PATH = $("#APP_ROOT_PATH_GEN").val();
	location.href = APP_ROOT_PATH + '/gen178/platform/info/platform_data/index' + '?agentViewFlag=' + $('#agentViewFlag').val()
}
...
$(function(){
	...
	var global = {
		...
		now: new Date(),
		go_partner_page_url: '/gen178/platform/about/partner/index' + '?agentViewFlag=' + $('#agentViewFlag').val()
	};
			...
			if(dd == 0) {
				if(ss >= 0){
					$(obj).prev('div.main_cdrtwo_span').html(hh + "时" + mm + "分" + ss + "秒后开始");
					 ...
					 $(obj).parent('.mian_cdrtwo1').next('.mf_ul').find('.md_btn').text('立即投资');
	            }
			} else {
