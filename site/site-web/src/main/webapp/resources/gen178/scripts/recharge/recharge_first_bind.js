...
$(document).ready(function() {
			...
			success: function (data) {
				if(data.hfDepGuideInfo.riskStatus == "no"){
					...
					if(pay_type == "quick_pay"){
						...
						//
						// location.href = app_root_path + "/gen178/recharge/eBankSubmit?amount="+$("#rechargeAmount").val()+"&bankId="+bankId;
						$.ajax({
							url: app_root_path + "/gen178/recharge/eBankSubmit",
							data: {
							...
							success: function(data) {
								if(data.resCode == '000000') {
									...
									location.href = app_root_path + "/gen178/recharge/error?resCode="+data.resCode+"&resMsg="+data.resMsg + '&agentViewFlag=' + $('#agentViewFlag').val();
								}
							}
						})
					}else{
    ...
    function comfirmPost(){
		...
		var url = app_root_path + "/gen178/recharge/submit_bind";
    	$.ajax({
    		url: url,
    		data:{
    		...
    		success: function(data) {
    			...
    			if(data.resCode == '000'){
    				location.href = app_root_path + "/gen178/recharge/recharge_success";
				} else {
					...
					setTimeout(function() {
						...
						$(formData).attr('action', app_root_path + '/gen178/recharge/recharge_index_submit');
	...
	function postData(){
		$.ajax({
			url: app_root_path + '/gen178/recharge/recharge_index_submit',
			data: {
				rechargeAmount: $("#rechargeAmount").val()
			},
			type: 'post',
	...
	function preTopUpSendSms(){
		...
		var url =  app_root_path + "/gen178/recharge/pre_submit_bind";
    	$.ajax({
    		url: url,
    		data:{
    			rechargeAmount:$("#rechargeAmount").val(),
