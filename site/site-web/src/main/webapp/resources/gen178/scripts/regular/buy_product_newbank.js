$(function(){
    ...
    function comfirmPost(){
		var url = app_root_path + "/gen178/regular/add_bankcard_submit";
    	...
    	$.ajax({
    		...
    		success: function(data) {
    			if(data.resCode == '000'){
    				location.href = app_root_path + "/gen178/regular/regular_success";
				} else {
					if(data.resMsg){
	...
	$(".z2_buut").on('click', function(){
		...
		location.href= app_root_path + "/gen178/regular/netBankbuySubmit?money="+buyAmount+"&productId="+productId;
	});
	// 网银确认支付结束
});
// 卡bin开始
