$(function() {
	...
	var global = {
		...
		forget_password_url: '/gen178/user/forget_password_first',
			...
			success: function (data) {
				...
				if(data.resCode == '000') {
					if(data.burl) {
						...
						location.href = global.base_url + global.index_url + '?agentViewFlag=' + global.agentViewFlag;
					}
				} else {
					$('#login_submit').off('click').on('click', function () {
						login();
					...
					if(constants.error_too_much == data.bsResCode) {
						...
						setTimeout(function() {
							location.href = global.base_url + global.forget_password_url + '?agentViewFlag=' + global.agentViewFlag;
						},1000);
