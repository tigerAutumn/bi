...
$(function(){
	...
	$("#savePassword").on('click',function(){
		...
		if(validatePwdForm()) {
			$.ajax({
				url: rootPath + "/gen178/profile/modifyloginpass",
				data: $('#pwdForm').serialize(),
				type: 'post',
				dataType: 'json',
				success: function(data) {
					if(data.resCode=='000'){
						drawAlerts("温馨提示","修改成功","确定",rootPath+"/gen178/assets/assets");
					} else {
						if(data.resMsg) {
	...
	$("#saveTrade").on('click',function(){
		...
		if(validateSetTradePwdForm()) {
			$.ajax({
				url: rootPath + "/gen178/profile/set_up_trader_password",
				data: $('#setTradeForm').serialize(),
				type: 'post',
				dataType: 'json',
				success: function(data) {
					if(data.resCode=='000'){
						drawAlerts("温馨提示","交易密码设置成功","确定",rootPath+"/gen178/assets/assets");
	...
	$("#updateTrade").on('click',function(){
		...
		if(validateForgetTradePassword()) {
			$.ajax({
				url: rootPath + "/gen178/profile/forget_trader_password",
				data: $('#updateTradeForm').serialize(),
				type: 'post',
				dataType: 'json',
				success: function(data) {
					if(data.resCode=='000'){
						drawAlerts("温馨提示","修改交易密码成功","确定",rootPath+"/gen178/assets/assets?safe=safe");
					} else if(data.resCode == '999') {
						drawToast(data.resMsg);
					} else {
						drawToast("港湾网络堵塞，请稍后再试！");
