...
$(function () {
    ...
    var global = {
        ...
        bind_card_index_url: $('#APP_ROOT_PATH_GEN').val() + '/gen178/bankcard/index?entry=' + $('#entry').val() + '&agentViewFlag=' + $('#agentViewFlag').val(),
        pre_bind_card_url: $('#APP_ROOT_PATH_GEN').val() + '/gen178/bankcard/pre',                  // 预绑卡地址
        bind_card_url: $('#APP_ROOT_PATH_GEN').val() + '/gen178/bankcard/bind',                     // 正式绑卡地址
        card_bin_url: $('#APP_ROOT_PATH_GEN').val() + '/gen178/regular/queryCardBank',              // 卡bin地址
        id_card_check_url: $("#APP_ROOT_PATH_GEN").val()+ "/gen178/regular/bankCardIs",             // 校验身份证地址
        next_assets_url: $('#APP_ROOT_PATH_GEN').val() + '/gen178/assets/assets' + '?agentViewFlag=' + $('#agentViewFlag').val(),                 // 我的账户链接
        next_self_url: $('#APP_ROOT_PATH_GEN').val() + '/gen178/assets/assets?selfbank=selfbank' + '&agentViewFlag=' + $('#agentViewFlag').val(),   // 我的银行卡链接
        next_top_up_url: $('#APP_ROOT_PATH_GEN').val() + '/gen178/assets/assets?recharge=recharge' + '&agentViewFlag=' + $('#agentViewFlag').val(), // 充值链接
        next_withdraw_url: $('#APP_ROOT_PATH_GEN').val() + '/gen178/assets/assets?withdraw=withdraw' + '&agentViewFlag=' + $('#agentViewFlag').val(),     // 提现链接
        
        next_invite_url: $('#APP_ROOT_PATH_GEN').val() + '/gen2.0/assets/assets?invite=invite',     // 邀请好友链接
        next_buy_url: $('#APP_ROOT_PATH_GEN').val() + '/gen178/regular/bind',                      // 购买链接
        ...
        if(global.entry == 'self') {
            ...
            setTimeout(function(){
                if(global.productId  && global.amount) {
                    location.href = global.next_buy_url + '?id=' + global.productId + '&money=' +global.amount + '&agentViewFlag=' + $('#agentViewFlag').val();
                } else {
                    location.href = global.next_self_url;
                }
            },2000);
