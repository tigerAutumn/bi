...
$(function(){
	...
	function sendCode(){
		...
		$.post($("#APP_ROOT_PATH_GEN").val()+"/gen178/identity/mobilecode",
			{
				mobile: mobile,
				verCode: verCode,
				type: 'exist'
	...
	function postDataForgetPassword(){
		...
		var password = $("#password").val();
		var password2 = $("#password2").val();
		var url =  global.base_url + "/gen178/user/passwordSubimt";
		$.ajax({
			url: url,
			data:{
				mobile:mobile,
			...
			success: function(data) {
				if(data.bsCode=='000'){
					...
					setTimeout(function(){
						location.href= global.base_url + "/gen178/index";
					},1500);
				}else{
