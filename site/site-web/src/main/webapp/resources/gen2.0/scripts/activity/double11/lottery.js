...
var lottery = {
	...
	// 开启抽奖
	_enable: function () {
		this.container.find('#jiangbtn').bind('click', this.beforeRoll);
	},

	// 禁用抽奖
	_disable: function () {
		this.container.find('#jiangbtn').unbind('click', this.beforeRoll);
	},
	...
	_stop: function () {
		...
		_this._enable();
		$("#showAwardContent").html("获得"+lucky_content);
	...
	beforeRoll: function () {
		...
		$.ajax({
			url: root_url + '/gen2.0/activity/1111/luckDraw',
			type: 'post',
			async: false,
				...
				if(data.resCode == '000000') {
					if(data.isStart != 'noStart' && data.isStart != 'end'){
						if(data.resCode =='9100049' || data.beforeTimes == 0) {
							...
							targetId = data.awardId - 12;
							lucky_content = data.awardContent;
					...
					if(data.isStart != 'noStart' && data.isStart != 'end'){
						...
						drawAlertClose('敬请期待', "<br>活动开始时间11月07日 00:00:01", null, "知道了", null);
						_this._enable();
	...
	_roll: function () {
		var _this = this;
		_this.container.find('[lottery-unit-index=' + _this._index() + '] div').css({'display':'none'});
		++_this.options.index;
		_this.container.find('[lottery-unit-index=' + _this._index() + '].lottery-unit div').css({'display':'block'});
