$(function(){
	...
	var submit_login = function(){
					...
					function(data){
						...
						if(data.resCode == '000'){
							...
							if(remind_or_buy == 'remind'){
								//提醒
								var url = root + "/gen2.0/regular/index?id="+$("#product_id").val()+"&to_remind=YES";
								location.href = url;
							}else{
								//购买
								$('.login_list').hide();
			...
			//													self.location.reload();
																location.href = $("#APP_ROOT_PATH_GEN").val() + "/gen2.0/regular/index?id="+$("#id").val();
															  }, 3100);
														}
													}
												}
...
function drawAlertRemindWindow(message,title,commitMessage,backMessage,commitUrl,backUrl){
	...
	if($(".alert_listtwo").length>0){
		...
		if('YES' == to_remind){
			var url = root + "/gen2.0/regular/index?id="+$("#product_id").val();
			location.href = url;
		}
		...
		success:function(data){
			$("body").append(data);
				...
				$(".alert_listtwo .alert_listtwo_btnl").click(function(){
					//添加短信通知
					$.ajax({
			    		url: $('#APP_ROOT_PATH_GEN').val() + "/gen2.0/regular/product_AddInform",
			    		data:{
			    			productId:$("#product_id").val()
			    		},
			    		success: function(data) {
...
function drawAlertConfirmNew4OldUser(message,title,commitMessage,backMessage,backUrl){
	...
	var url=$('#APP_ROOT_PATH_GEN').val() +'/gen2.0/static/alert_confirm_new_1';
	$.ajax({
		type:"GET",
		url:url,
		data:"",
