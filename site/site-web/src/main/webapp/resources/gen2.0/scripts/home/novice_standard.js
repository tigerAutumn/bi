window.onload = function() {
	...
	function move() {
		...
		i++
		for (j = 0; j < aImg.length; j++) {
			aImg[j].style.background = 'url('+APP_ROOT_PATH+'/resources/gen2.0/images/home/mth3.png) no-repeat center'
			aImg1[j].style.display = 'none'
		}
		if (i == aImg.length) {

			aImg[0].style.background = 'url('+APP_ROOT_PATH+'/resources/gen2.0/images/home/mth2.png) no-repeat center'
			aImg1[0].style.display = 'block'
		} else {

			aImg[i].style.background = 'url('+APP_ROOT_PATH+'/resources/gen2.0/images/home/mth2.png) no-repeat center'
...
function clickRed(id){
	location.href = $("#APP_ROOT_PATH").val() + "/gen2.0/regular/index?id="+id;
}
...
function clickLogin(){
	var user_id = $("#user_id").val();
	if(user_id == null || user_id == ""){
		location.href = $("#APP_ROOT_PATH").val() + "/gen2.0/user/login/index";
	}else{
		drawToast("您已经登录币港湾！");
	}
}
