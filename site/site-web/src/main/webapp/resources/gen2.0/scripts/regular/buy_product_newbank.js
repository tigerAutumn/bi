$(function(){
    ...
    function comfirmPost(){
		var url = app_root_path + "/gen2.0/regular/add_bankcard_submit";
    	...
    	$.ajax({
    		...
    		success: function(data) {
    			if(data.resCode == '000'){
    				location.href = app_root_path + "/gen2.0/regular/regular_success";
				} else {
					if(data.resMsg){
	...
	$(".z2_buut").on('click', function(){
		var buyAmount = $("#buy_money").val();
		var productId = $("#id").val();
		location.href= app_root_path + "/gen2.0/regular/netBankbuySubmit?money="+buyAmount+"&productId="+productId;
	});
	// 网银确认支付结束
});
// 卡bin开始
...
function cardbin() {
	...
	$.post(root+"/gen2.0/regular/queryCardBank",{cardNo:cardNo},function(data){
		if(data.bin.bankId) {
			$("#bankName").find("option").each(function(index, domEle){
				if(data.bin.bankId == $(domEle).attr("value")) {
					$("#bankName").val(data.bin.bankId);
					$("#onelimit").text(parseFloat($(domEle).attr("oneTop")));
					$("#daylimit").text(parseFloat($(domEle).attr("dayTop")));
					$("#limit").show("slow");
					backup = cardNo;
					return false;
				}
