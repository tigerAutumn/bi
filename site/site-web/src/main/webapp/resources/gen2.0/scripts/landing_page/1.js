...
$(function(){
	...
	$(".red_packet_btn").on('click', function() {
		$.ajax({
			url: global.root_path + '/gen2.0/user/login/isInLogin',
			type: 'post',
			success: function(data) {
				if(data.isInLogin) {
					drawAlertNew('您已经是币港湾注册用户');
				} else {
					$('#ly').show();
					$('#hike_login_copy').show();
				}
			}
		});
	});
	...
	$(".new_buyer_btn").on('click', function() {
		$.ajax({
			...
			success: function(data) {
				if(data.isInLogin) {
					if($('#id').val()) {
						location.href = global.root_path + "/gen2.0/regular/index?id="+$('#id').val();
					} else {
						drawToast('当前新手标已结束');
					}
				} else {
