...
$(document).ready(function(){
	...
	//微信滑出
        $('.wx').hover(function(){
        	$('.imgs').stop(true).fadeIn(100);
        },function(){
        	$('.imgs').stop(true).hide(100);
        });
        ...
        $(".APP_img").hover(function(){
        	$(".APP_wp").stop(true).show(200);
        },function(){
        	$(".APP_wp").stop(true).hide(200);
        })
        //微信固定
        function fadeshow(cla){
        	$(cla).css("background-color","#FF6633");
        	$(cla).fadeIn(100);
        }
        function fadeout(cla){
        	$(cla).stop(true,true).hide(100);
        	$(cla).css("background-color","transparent")
        }
        $(".f5").hover(function(){
        	fadeshow(".f5_left");
        },function(){
    ...
    /*侧导航-s*/
        ...
        // 回款路径下银行卡点击选中
        $('.yuan').click(function(){
                $(this).css('background','url('+$("#APP_ROOT_PATH_GEN").val()+'/resources/gen2.0/images/duih.png)');
        ...
        function shownnumber(data,id){
            ...
            for(var i=0;i<one_ko.length;i++){
                for(var j=0;j<all.length;j++){
                    if(one_ko[i]==all[j]){
                        var img_url = rootPath+"/resources/gen2.0/images/"+all[j]+".png";
