$(function(){
	...
	$("#remind_me").click(function(){
		...
		//短信提醒
		$.ajax({
			url: global.root_path + '/gen2.0/activity/weekhay_addRemind',
			type: 'post',
			success: function(data) {
				if(data.resCode == '000'){
					...
					location.href = $("#APP_ROOT_PATH_GEN").val() + "/gen2.0/user/login/index?burl=/gen2.0/activity/weekhay_index";
				}
			},
			error: function(data) {
...
function button_change(button_num,change,num){
	if(change == "buying"){
		setTimeout( function(){
			$("#button_"+button_num+"").removeClass("ready");
			$("#button_"+button_num+"").text("查看秒杀标的");
		},num);
	}else if(change == "end"){
		setTimeout( function(){
			$("#button_"+button_num+"").addClass("finished");
