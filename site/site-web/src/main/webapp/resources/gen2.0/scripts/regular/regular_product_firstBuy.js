$(function(){
	...
	function sendCode(){
		...
		$.post($("#APP_ROOT_PATH_GEN").val()+"/gen2.0/regular/firstBuySubmitPro",
		{productId:id,
		amount:buy_money,
		userName:userName,
		idCard:idCard,
		...
		function(data){
			if(data.resCode=='000'){
				...
				if(data.resCode == '931014'){
					...
					var url_over = app_root_path+"/gen2.0/assets/assets?recharge=recharge";
    ...
    function postDataRegister(){
		...
		var redPacketId = $("#redPacketId").val();
		var url =  $("#APP_ROOT_PATH_GEN").val()+ "/gen2.0/regular/firstBuySubmit";
    	$.ajax({
    		url: url,
    		data:{
    			productId:id,
