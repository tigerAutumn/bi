...
function matchDetail(e){
	var root = $("#APP_ROOT_PATH_GEN").val();
	$.ajax({
		url: root + "/gen178/match/myMatchDetailZan",
		data:{
			matchId:e
		},
...
function loanPrint(obj){
	window.open($("#APP_ROOT_PATH_GEN").val()+"/gen2.0/agreement/loanAgreement?"+$(obj).attr('data-datas'),'_blank','width=800,height=600,menubar=no,toolbar=no, location=no,directories=no,status=no,scrollbars=yes,resizable=yes');
}

//打开债权转让协议
function claimsPrint(obj){
	window.open($("#APP_ROOT_PATH_GEN").val()+"/gen2.0/agreement/agreementClaims?"+$(obj).attr('data-datas'),'_blank','width=800,height=600,menubar=no,toolbar=no, location=no,directories=no,status=no,scrollbars=yes,resizable=yes');
}
