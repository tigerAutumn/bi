$(function(){
	...
	var close_award = function(wp) {
		//$(wp).fadeOut(600);
		location.href = root + "/gen2.0/activity/luckyDrawIndex";
	};
	// 抽奖结果按钮操作 2.没有中奖
...
//		location.href = root + "/gen2.0/activity/luckyDrawIndex";
	...
	var luckyDraw = function(activityBatchNo){
		$.ajax({
			url: root + "/gen2.0/activity/luckyDraw",
			type: 'post',
			dataType: 'json',
			data:{
	...
	var checkMobile = function() {
		...
		$.ajax({
			url: root + "/gen2.0/user/login/mobileRegisted",
			type: 'post',
			dataType: 'json',
			data: {
				mobile: mobile
			...
			success: function(data) {
				if(data.resCode == '000') {
					...
					// 未注册
						...
						$.ajax({
							url: root + "/gen2.0/identity/mobilecode",
							data: {
								mobile: data.mobile
							},
							type:"post",
	...
	$("#login_btn").click(function(){
		...
		var url = root + "/gen2.0/user/login";
	...
	$("#register_btn").click(function(){
		...
		var url = root + "/gen2.0/user/register_submit";
	...
	$('.invite_btn').click(function(){
		window.open(root + '/gen2.0/assets/assets?invite=invite','');
	});
	// 邀请用户结束
	// 继续投资赢取砸蛋机会开始
	$('.invest_btn').click(function(){
		window.open(root + '/gen2.0/regular/list','');
	});
	// 继续投资赢取砸蛋机会结束
