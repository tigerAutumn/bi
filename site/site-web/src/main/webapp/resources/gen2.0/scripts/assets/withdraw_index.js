$(function () {
	...
	var global = {
		...
		assets_url: $("#APP_ROOT_PATH_GEN").val() + "/gen2.0/assets/assets",
		...
		withdraw_index_url: $("#APP_ROOT_PATH_GEN").val() + "/gen2.0/withdraw/withdraw_deposit",			// 提现首页
		withdraw_url: $("#APP_ROOT_PATH_GEN").val() + '/gen2.0/withdraw/withdraw_submit',					// 提现链接
		set_pay_passwrod: $("#APP_ROOT_PATH_GEN").val() + '/gen2.0/profile/set_up_trader_password',			// 设置交易密码
		check_day_limit_url: $("#APP_ROOT_PATH_GEN").val() + '/gen2.0/withdraw/checkDayLimit',
	...
	function setPayPwd() {
		...
		if(reg.test($('#payPassword').val())){
			drawToast('交易密码格式有误！');
			return false;
		}
		if(reg.test($('#curPassword').val())){
			drawToast('确认密码格式有误！');
			return false;
		}
		// 未设置交易密码，先设置交易密码
