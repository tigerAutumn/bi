...
//		self.ctx.strokeStyle = 'hsla('+this.hue+', 100%, '+this.brightness+'%, '+this.alpha+')';
		self.ctx.strokeStyle='rgba(255,255,0,1)';//烟花
		self.ctx.stroke();				
		...
		if(self.flickerDensity > 0){
			...
			if(rand(0, inverseDensity) === inverseDensity){
				...
				self.ctx.fillStyle='rgba(255,200,0,1)';//烟花
				self.ctx.fill();
			}	
		}
	};
	...
	Firework.prototype.draw = function(){
		...
		self.ctx.closePath();
		self.ctx.strokeStyle = 'hsla('+this.hue+', 100%, '+this.brightness+'%, '+this.alpha+')';
		self.ctx.strokeStyle='rgba(200,255,0,1)';//发射的
		self.ctx.stroke();	
		...
		if(self.showTarget){
			...
			self.ctx.strokeStyle='rgba(255,0,0,0)';//圈圈
			self.ctx.stroke();
			self.ctx.restore();
		}
		...
		if(self.showShockwave){
			...
			self.ctx.beginPath();
			self.ctx.arc(0, 0, 1*(10), 0, Math.PI, true);
