$(function(){
	...
	var global = {
		...
		withdraw_url: $("#APP_ROOT_PATH").val() + '/micro2.0/withdraw/withdraw_submit_qb',				// 提现链接
		...
		wfrom: $('#wfrom').val(),
		backUrl: $('#backUrl').val(),
		qp178_withdraw_url:	$('#backUrl').val() ? $('#backUrl').val() : $("#APP_ROOT_PATH").val() + '/micro2.0/regular/index',
		withdraw_result_url: $("#APP_ROOT_PATH").val() + '/withdraw/withdraw_result_page',
	};
	// ============================ 函数 =====================================
	...
	function preWithdraw(no_alert) {
		...
		// 用户状态是否冻结中校验
		if (global.userStatus == "4") {
			drawToast("您账户已冻结，请联系客服详询");
			return false;
		}
			...
			success: function (data) {
				if (data.resCode == '000') {
					if(data.HavePayPassword == 'yes'){
						...
						if(global.qianbao) {
							location.href = global.modify_pay_password_url + "?from=withdraw&amount=" + amount + "&qianbao=qianbao&agentViewFlag="+agentViewFlag+"&wfrom="+global.wfrom+"&backUrl="+global.backUrl;
						} else {
							location.href = global.modify_pay_password_url + "?from=withdraw&amount=" + amount + "&wfrom="+global.wfrom+"&backUrl="+global.backUrl;
						}
					}
				}
			}
					...
					if(!data.truePayPassword) {
						...
						if(data.failNums >= 6){
							if(global.qianbao && global.qianbao == 'qianbao') {
								drawAlerts('提示', "交易密码多次输入错误，请登录币港湾官网找回" , "确定", global.withdraw_index_url + '?qianbao=qianbao&agentViewFlag='+$('#agentViewFlag').val()+"&wfrom="+global.wfrom+"&backUrl="+global.backUrl);
							} else {
								drawAlerts('提示', "交易密码多次输入错误，请登录币港湾官网找回", "确定", global.withdraw_index_url +"?wfrom="+global.wfrom+"&backUrl="+global.backUrl);
							}
						} else {
							if((5-data.failNums) == 0) {
								...
								if(global.qianbao && global.qianbao == 'qianbao') {
									drawAlerts('提示', "交易密码有误，请重新输入（还有"+(5-data.failNums)+"次机会）" , "重试", global.withdraw_index_url + '?qianbao=qianbao&agentViewFlag='+$('#agentViewFlag').val()+"&wfrom="+global.wfrom+"&backUrl="+global.backUrl);
								} else {
									drawAlerts('提示', "交易密码有误，请重新输入（还有"+(5-data.failNums)+"次机会）" , "重试", global.withdraw_index_url +"?wfrom="+global.wfrom+"&backUrl="+global.backUrl );
								}
							}
						...
						// 交易密码正确						
//						var url = global.qp178_withdraw_url;	
						
						$.ajax({
							url: global.withdraw_url,
							type: 'get',
							dataType: 'json',
							data: {
								amount: amount,
							...
							success: function (data) {
								closeDrawDiv();
								openDrawDiv("币港湾已收到您的提现申请，正在处理中(结果将以短信通知)");
								if (data.resCode == '000') {
...
//							}
//							form.append("<input name='wfrom' value='" + $('#wfrom').val() + "'/>");
//							form.append("<input name='backUrl' value='" + $('#backUrl').val() + "'/>");
//							form.appendTo("body");
//							alertHide('.close');
//							form.submit();
//						
