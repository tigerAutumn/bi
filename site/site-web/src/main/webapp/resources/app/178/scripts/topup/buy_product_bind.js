...
/*	//验证是否单笔超限
	...
	//$(".top_line").css("background-color","#ff6633");
	//正式下单
	if(type == 'regular') {
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data.errorCode == '000' || data.errorCode == '000000') {
					openDrawDiv("币港湾已收到您的充值申请，正在处理中，请稍等（结果将以短信通知）");
					var backUrl = $('#backUrl').val();
					var source = $('#source').val();
					if(source == 'buy') {
						var url = root+"/micro2.0/regular/bind?id=" + $('#id').val() + '&from='+$('#from').val() + '&amount='+$('#buyAmount').val();
	...
	//预下单
		...
		var keyList = ['from', 'id', 'buyAmount', 'source'];
