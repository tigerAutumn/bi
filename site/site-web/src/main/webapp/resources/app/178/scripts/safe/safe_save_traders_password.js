				...
				if (data.resCode == '000') {
					if(from == 'withdraw') {
						if(qianbao && qianbao=='qianbao') {
							location.href = $('#APP_ROOT_PATH').val() + '/micro2.0/withdraw/withdraw_deposit?qianbao=qianbao&from=pwd&amount=' + amount + '&agentViewFlag='+agentViewFlag+"wfrom="+wfrom+"&backUrl="+backUrl;
						} else {
							location.href = $('#APP_ROOT_PATH').val() + '/micro2.0/withdraw/withdraw_deposit?from=pwd&amount=' + amount+"&wfrom="+wfrom+"&backUrl="+backUrl;
						}
					} else {
						if(qianbao && qianbao=='qianbao') {
